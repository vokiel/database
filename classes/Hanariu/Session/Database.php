 ...
 */
class Database extends \Hanariu\Session {
  ...
  public function __construct(array $config = NULL, $id = NULL)
  {
    ...
    // Load the database
    $this->_db = \Hanariu\Database::instance($config['group']);
  ...
  {
    if ($id OR $id = \Hanariu\Cookie::get($this->_name))
    {
      $result = \Hanariu\DB::select(array($this->_columns['contents'], 'contents'))
        ->from($this->_table)
  ...
  protected function _regenerate()
  {
    ...
    $query = \Hanariu\DB::select($this->_columns['session_id'])
      ->from($this->_table)
      ->where($this->_columns['session_id'], '=', ':id')
      ->limit(1)
      ->bind(':id', $id);
  ...
  protected function _write()
  {
    ...
    {
      // Insert a new row
      $query = \Hanariu\DB::insert($this->_table, $this->_columns)
        ->values(array(':new_id', ':active', ':contents'));
    }
    else
    {
      // Update the row
      $query = \Hanariu\DB::update($this->_table)
    ...
    \Hanariu\Cookie::set($this->_name, $this->_session_id, $this->_lifetime);
  ...
  protected function _destroy()
  {
    ...
    $query = \Hanariu\DB::delete($this->_table)
      ->where($this->_columns['session_id'], '=', ':id')
